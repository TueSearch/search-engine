...
def spacy_tokenize(text: str) -> list[Token]:
    ...
    tokens = [token for token in tokens if len(
        token.text) < REMOVE_LONG_WORD_THRESHOLD]
...
def do_text_contain_english_content(text: str) -> bool:
    ...
    # First try a more sophisticated lib
    try:
        langs = Detector(text).languages
        for language in langs:
            lang_name = str(language.name)
            if lang_name == "English":
                conf = float(language.confidence)
                is_english = conf > CRAWL_ENGLISH_CLASSIFICATION_THRESHOLD
                return is_english
        return False
...
def generate_text_document_from_html(html_content: str) -> 'Document':
    ...
    document.body = make_text_human_readable(
        soup.body.get_text(separator=" ") if soup.body else "")
    document.body_tokens = advanced_tokenize_with_pos(document.body)

    document.title = make_text_human_readable(
    ...
    for meta_tag in meta_tags:
        if 'name' in meta_tag.attrs and 'content' in meta_tag.attrs:
            ...
            if name == 'description':
                ...
                document.meta_description_tokens = advanced_tokenize_with_pos(
                    document.meta_description)
            elif name == 'keywords':
                document.meta_keywords = make_text_human_readable(content)
                document.meta_keywords_tokens = advanced_tokenize_with_pos(
                    document.meta_keywords)
            elif name == 'author':
                document.meta_author = make_text_human_readable(content)
                document.meta_author_tokens = advanced_tokenize_with_pos(
                    document.meta_author)

    # Set h1, h2, h3, h4, h5, h6 fields
    h1_tags = soup.find_all('h1')
    document.h1 = make_text_human_readable(
        ' '.join([h1.get_text().strip() for h1 in h1_tags]))
    document.h1_tokens = advanced_tokenize_with_pos(document.h1)

    h2_tags = soup.find_all('h2')
    document.h2 = make_text_human_readable(
        ' '.join([h2.get_text().strip() for h2 in h2_tags]))
    document.h2_tokens = advanced_tokenize_with_pos(document.h2)

    h3_tags = soup.find_all('h3')
    document.h3 = make_text_human_readable(
        ' '.join([h3.get_text().strip() for h3 in h3_tags]))
    document.h3_tokens = advanced_tokenize_with_pos(document.h3)

    h4_tags = soup.find_all('h4')
    document.h4 = make_text_human_readable(
        ' '.join([h4.get_text().strip() for h4 in h4_tags]))
    document.h4_tokens = advanced_tokenize_with_pos(document.h4)

    h5_tags = soup.find_all('h5')
    document.h5 = make_text_human_readable(
        ' '.join([h5.get_text().strip() for h5 in h5_tags]))
    document.h5_tokens = advanced_tokenize_with_pos(document.h5)

    h6_tags = soup.find_all('h6')
    document.h6 = make_text_human_readable(
        ' '.join([h6.get_text().strip() for h6 in h6_tags]))
